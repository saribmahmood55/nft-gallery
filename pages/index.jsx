...
const Home = () => {
    ...
    const fetchNFTsForOwners = async(tokenId) => {
        let nfts;
        console.log("fetching nfts");
        const api_key = "ppG4iGGKu8-vv2IuSYaYuWpT1EB_wKk_"
        ...
        setNextToken(nfts.nextToken? nfts.nextToken : null);
    }
    ...
    const fetchNFTs = (tokenId, isForCollection) => {
        if (isForCollection)
            fetchNFTsForCollection(tokenId)
        else
            fetchNFTsForOwners(tokenId)
    }
        ...
        <div className="flex flex-col items-center justify-center py-8 gap-y-3">
            <div className="flex flex-col w-full justify-center items-center gap-y-2">
                ...
                <button className={"disabled:bg-slate-500 text-white bg-blue-400 px-4 py-2 mt-3 rounded-sm w-1/5"} onClick={
                    () => fetchNFTs("0", fetchForCollection)
                }>Let's go! </button>
            </div>
            ...
            {
                !!nextToken && (<button className={"disabled:bg-slate-500 text-white bg-blue-400 px-4 py-2 mt-3 rounded-sm w-1/5"}
                        onClick={() => fetchNFTs(nextToken, fetchForCollection)}>Next Page</button>)
            }
        </div>
    )
}
